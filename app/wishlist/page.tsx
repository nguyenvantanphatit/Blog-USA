"use client"
import { deleteWishlist, getWishlist } from '@/lib/api'
...
export default function WishlistPage() {
    ...
    const handleDelteWishList = async (productId : number) => {
        await deleteWishlist(productId);
        setDataWishlist((wishList) => wishList.filter((wish) => wish.id !== productId));
            ...
            <section className="bg-gray-50 py-8 antialiased dark:bg-gray-900 md:py-12">
                <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
                    ...
                    <div className="mb-4 grid gap-4 sm:grid-cols-2 md:mb-8 lg:grid-cols-3 xl:grid-cols-4">
                        {dataWishlist.map((product: any, index: any) => (
                            <div key={index} className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                <div className="h-56 w-full">
                                        <img className="mx-auto h-full" src={product?.Product?.imageUrl || "/Featured/gioqua.jpg"} alt="" />
                                </div>
                                <div className="pt-6">
                                    <div className="mb-4 flex items-center justify-between gap-4">
                                        ...
                                        <div className="flex items-center justify-end gap-1">
                                            ...
                                            <button onClick={() => handleDelteWishList(product.id)} type="button" className="rounded-lg p-2 text-gray-500 hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                                             Delete
                                            </button>
                                        </div>
                                    </div>
                                    <Link href="#" className="text-lg font-semibold leading-tight text-gray-900 hover:underline dark:text-white">{product.name}</Link>
